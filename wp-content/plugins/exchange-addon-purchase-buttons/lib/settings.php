...
function it_exchange_purchase_buttons_addon_print_settings() {
	...
	<div class="wrap">
		...
		<h2><?php _e( 'Purchase Button Default Settings', 'it-l10n-exchange-addon-purchase-buttons' ); ?></h2>
		...
		if ( ! empty( $_POST['__it-form-prefix'] ) && 'it-exchange-addon-purchase-buttons' == $_POST['__it-form-prefix'] )
			ITUtility::show_status_message( __( 'Options Saved', 'it-l10n-exchange-addon-purchase-buttons' ) );

		?>
		<p><?php _e( 'You can override the below settings on a per-product basis in the Advanced tab of the New / Edit Product screen for each product.', 'it-l10n-exchange-addon-purchase-buttons' ); ?></p>
		...
		<ul class="puchase-button-settings">
			<li class="top-level-setting">
				<label for="disable-buy-now">
					...
					$form->add_check_box( 'disable-buy-now', $field_options ); ?> <?php _e( 'Disable Buy Now buttons', 'it-l10n-exchange-addon-purchase-buttons' );
					?>
				</label>
				<ul class="disable-buy-now-settings <?php esc_attr_e( $disable_buy_now_class ); ?>">
					<li>
						<label for="disable-buy-now-scope-globally">
							...
							$form->add_radio( 'disable-buy-now-scope', $radio_options ); ?> <?php _e( 'Disable Buy Now globally', 'it-l10n-exchange-addon-purchase-buttons' );
							?>
						</label>
					</li>
					<li>
						<label for="disable-buy-now-scope-product-type">
							...
							if ( ! empty( $disable_buy_now_class ) ) { $radio_options['disabled'] = 'disabled'; }
							$form->add_radio( 'disable-buy-now-scope', $radio_options ); ?> <?php _e( 'Disable Buy Now for specific product types', 'it-l10n-exchange-addon-purchase-buttons' );
							?>
						</label>
			...
			<li class="top-level-setting">
				<label for="disable-add-to-cart">
					...
					$form->add_check_box( 'disable-add-to-cart', $field_options ); ?> <?php _e( 'Disable Add to Cart buttons', 'it-l10n-exchange-addon-purchase-buttons' );
					?>
				</label>
				<ul class="disable-add-to-cart-settings <?php esc_attr_e( $disable_add_to_cart_class ); ?>">
					<li>
						<label for="disable-add-to-cart-scope-globally">
							...
							$form->add_radio( 'disable-add-to-cart-scope', $radio_options ); ?> <?php _e( 'Disable Add to Cart globally', 'it-l10n-exchange-addon-purchase-buttons' ); ?>
						</label>
					</li>
					<li>
						<label for="disable-add-to-cart-scope-product-type">
							...
							if ( ! empty( $disable_add_to_cart_class ) ) { $radio_options['disabled'] = true; }
							$form->add_radio( 'disable-add-to-cart-scope', $radio_options ); ?> <?php _e( 'Disable Add to Cart for specific product types', 'it-l10n-exchange-addon-purchase-buttons' );
							?>
						</label>
